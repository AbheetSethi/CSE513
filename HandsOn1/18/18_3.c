...
int main()
{
	...
	struct flock lock;
	lock.l_type = F_RDLCK;  // Implementing read lock
	...
	{
		perror("Failed to lock the file for reading");
		close(fd);
		return 1;
	}
	...
	printf("Before entering critical section (READ LOCK)\n");
	...
	printf("Read operation completed. No changes made to ticket count.\n");
